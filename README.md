...
- The "PRODUCT" column determines what type of parameter the value in the "VALUE" column refers to. There are a total of 27 different types of "PRODUCT" and some examples are "Total exports", "Distribution losses", "Wind", "Oil", "Used for pumped storage", "Renewables", etc. However, the only ones of interest for the study are "Net electricity production" and "Solar" so we will need to filter the data in order to only get the results for these two types of "PRODUCT".
- The "share" column shows the percentage of the "Net electricity production" of each value in the VALUE column (for the rows where PRODUCT = "Net electricity production", the value in the column "share" is 1 as it represents the 100%). Since we want to obtain the data annually instead of monthly, this column will not help us as much as we might intuitively think. If we were to average the monthly values in this column we would not get the real annual share value. The value corresponding to the month in which the total electricity production is the maximum should have more weight in the annual average than the value corresponding to the month in which the total energy production is the minimum. The way to correctly make the annual percentage is to add up all the monthly values and then make the percentage of the two totals.
